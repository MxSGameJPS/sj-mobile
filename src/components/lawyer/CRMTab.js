...
export default function CRMTab({
  ...
  const checkCrmLimit = () => {
    ...
    if (planType === "START" && clients.length >= 10) {
      if (onPlanUpgrade) {
        onPlanUpgrade(
          "Você atingiu o limite de 10 clientes do Plano START. Faça upgrade para o Plano PRO para gerenciar clientes ilimitados.",
        );
      } else {
        Alert.alert(
          "Limite Atingido",
          "Você atingiu o limite de 10 clientes do Plano START. Faça upgrade para continuar.",
        );
      }
      return false;
    }
  ...
  const fetchClients = useCallback(async () => {
    ...
    try {
      ...
      console.error("[CRM] fetchClients:", e);
    }
  }, [userId, accessToken]);
  ...
  const fetchFinancials = useCallback(async () => {
    ...
    try {
      ...
      console.error("[CRM] fetchFinancials:", e);
    }
  }, [userId, accessToken]);
  ...
  const loadDossierTab = async (tab, client) => {
    ...
    if (tab === "Interações") {
      ...
      try {
        ...
        console.error("[CRM] interactions:", e);
      }
    ...
    if (tab === "Análise IA") {
      ...
      setInsight("");
      try {
        const res = await fetch(`${WEB_API}/crm/insights?clientId=${c.id}`, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  const saveFinance = async () => {
    ...
    try {
      const payload = {
        ...
        due_date:
          financeForm.due_date || new Date().toISOString().split("T")[0],
        created_at: new Date().toISOString(),
        paid_at:
          financeForm.status === "PAGO" ? new Date().toISOString() : null,
      };
    ...
    try {
      ...
      if (!data.success) {
        if (data.error_type === "QUOTA_EXCEEDED") {
          Alert.alert(
            "Limite Atingido",
            "Você atingiu o limite de clientes do seu plano. Faça upgrade para continuar.",
          );
        } else {
          Alert.alert(
            "Erro",
  ...
  const handleExtractPhoto = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        ...
        "Permita o acesso à galeria para usar esta função.",
      );
      return;
    }
    ...
    try {
      ...
      if (data.success && data.data) {
        setForm((prev) => ({
          ...
          endereco_completo:
            data.data.endereco_completo || prev.endereco_completo,
          email: data.data.email || prev.email,
        }));
        setShowNewClient(true);
        Alert.alert(
          "✅ Extração Concluída",
          "Dados extraídos com sucesso! Revise e confirme o formulário.",
        );
      } else {
        Alert.alert(
          "Erro",
          data.message || "Não foi possível extrair dados do documento.",
        );
      }
    } catch (e) {
  ...
  const startRecording = async () => {
    try {
      ...
      if (permission.status !== "granted") {
        Alert.alert(
          "Permissão Necessária",
          "Permita o acesso ao microfone para gravar o comando de voz.",
        );
        return;
      }
  ...
  const processAudioCommand = async (uri) => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        Alert.alert(
          "✅ Voz Processada",
          "Dados extraídos com sucesso! Revise e salve o formulário.",
        );
      } else {
        Alert.alert(
          "Atenção",
          data.message ||
            "A IA não conseguiu entender ou extrair os dados do áudio. Tente novamente.",
        );
      }
    } catch (e) {
      ...
      Alert.alert(
        "Erro",
        "Erro ao conectar ou enviar áudio para processamento.",
      );
    } finally {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        Alert.alert(
          "Atenção",
          data.message ||
            "A IA não conseguiu extrair os dados. Tente descrever mais detalhadamente.",
        );
      }
    } catch (e) {
  ...
  const pct =
    financials.previsto > 0
      ? Math.min(
          100,
          Math.round((financials.recebido / financials.previsto) * 100),
        )
      : 0;
      ...
      >
        ...
        <View style={s.actionRow}>
          <TouchableOpacity
            ...
            onPress={() => {
              if (checkCrmLimit()) setShowVoice(true);
            }}
          >
            <Feather name="mic" size={14} color="#ff453a" />
            <Text style={[s.actionTxt, { color: "#ff453a" }]}>
              Comando{"\n"}de Voz
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            ...
            <Text style={[s.actionTxt, { color: "#a3a9c2" }]}>
              {extracting ? "Extraindo..." : "Extração IA"}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.actionBtn, s.btnGold]}
            onPress={() => {
              if (checkCrmLimit()) {
                resetForm();
                setShowNewClient(true);
          ...
          >
            ...
            <Text style={[s.actionTxt, { color: "#f5c853" }]}>
              Novo{"\n"}Cliente
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={s.infoBox}>
          <View style={s.infoIcon}>
            <Feather name="shield" size={20} color="#f5c853" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={s.infoTitle}>O que você pode fazer:</Text>
            <Text style={s.infoText}>
              Gerencie sua carteira com IA. Analise risco, segmente clientes,
              acompanhe histórico e receba recomendações estratégicas. Tudo
              centralizado em um dossiê completo para cada cliente.
            </Text>
            <View style={s.tagRow}>
              {[
                "Score de Confiança",
        ...
        <View style={s.finCard}>
          ...
          <View style={s.finGrid}>
            ...
            <View style={s.finStat}>
              ...
              <Text style={[s.finVal, { color: "#10b981" }]}>
                {currency(financials.recebido)}
              </Text>
            </View>
          </View>
          <View style={s.progSection}>
            ...
            <Text style={s.finCount}>
              {financials.count} lançamentos financeiros identificados neste
              período.
            </Text>
          </View>
        </View>
          ...
          <View style={s.sheet}>
            ...
            <ScrollView keyboardShouldPersistTaps="handled">
              ...
              {[
                ...
                {
                  label: form.tipo === "Pessoa Física" ? "CPF" : "CNPJ",
                  key: "cpf_cnpj",
                  placeholder:
                    form.tipo === "Pessoa Física"
                      ? "000.000.000-00"
                      : "00.000.000/0000-00",
                ...
                {
                  label:
                    form.tipo === "Pessoa Física" ? "RG" : "Inscrição Estadual",
                  key: "rg_ie",
              ...
              >
                {saving ? (
                  ...
                  <Text style={s.saveBtnTxt}>Salvar Cliente</Text>
                )}
              </TouchableOpacity>
            ...
            >
              <View style={s.voiceInfoBox}>
                ...
                <Text style={s.voiceInfoTxt}>
                  Grave um áudio ou digite o comando descrevendo os dados do
                  cliente naturalmente (Nome, CPF, E-mail, Profissão, etc). A IA
                  estruturará tudo.
                </Text>
              </View>
                ...
                {processingVoice ? (
                  <View style={s.idleMicContainer}>
                    ...
                    <Text style={s.recordingHint}>
                      Transcrevendo áudio e extraindo dados do cliente...
                    </Text>
                  </View>
                ) : isRecording ? (
                  <View style={s.recordingPulseContainer}>
                    ...
                    <Text style={s.recordingHint}>
                      Toque no quadrado para parar e processar
                    </Text>
                  </View>
                ) : (
                  <View style={s.idleMicContainer}>
                    ...
                    <Text style={s.recordingHint}>
                      Clique para iniciar o comando por voz
                    </Text>
                  </View>
                )}
              </View>
              ...
              <View style={[s.voiceActions, { marginTop: 10 }]}>
                ...
                {!isRecording ? (
                  <TouchableOpacity
                    style={[
                      s.voiceProcessBtn,
                      (!voiceText.trim() || processingVoice) && {
                        opacity: 0.5,
                      },
                    ]}
                  ...
                  >
                    {processingVoice ? (
                      ...
                      <>
                        ...
                        <Text style={[s.voiceProcessTxt, { color: "#fff" }]}>
                          Processar Áudio
                        </Text>
                      </>
                    )}
          ...
          <View style={[s.sheet, { height: "90%" }]}>
                  ...
                  >
                    ...
                    >
                      <Text
                        style={[
                          s.riskPillTxt,
                          { color: riskColor(selectedClient.risk_score || 50) },
                        ]}
                      >
                        {riskLabel(selectedClient.risk_score || 50)} (
                        {selectedClient.risk_score || 50}%)
                      </Text>
                    </View>
            ...
            >
              {DOSSIER_TABS.map((t) => (
                ...
                >
                  <Text
                    style={[s.tabBtnTxt, dossierTab === t && s.tabBtnTxtActive]}
                  >
                    {t}
                  </Text>
                </TouchableOpacity>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.clientContact}>
            {client.email || "Sem e-mail"} • {maskPhone(client.phone)}
          </Text>
        </View>
      </View>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View
            style={[
              s.riskPill,
              {
                borderColor: riskColor(client.risk_score || 50),
          ...
          >
            <Text
              style={[
                s.riskPillTxt,
                { color: riskColor(client.risk_score || 50) },
              ]}
            >
...
function DossierGeral({ client }) {
  const rows = [
    ...
    { icon: "file-text", label: "CPF / CNPJ", value: maskDoc(client.cpf_cnpj) },
    {
      icon: "file",
      label: "RG / IE",
      value: client.rg ? `***${String(client.rg).slice(-3)}` : "--",
    },
    {
      icon: "heart",
      label: "Estado Civil",
  ...
  return (
    <View style={s.dSection}>
      <View style={s.dossierScoreCard}>
        ...
        <Text style={s.dossierScoreNum}>
          {client.risk_score || 50}% de risco litigioso
        </Text>
      </View>
      {rows.map((r) => (
        <View key={r.label} style={s.dRow}>
          <View style={s.dRowIcon}>
            <Feather name={r.icon} size={14} color="#606672" />
          </View>
          <View style={{ flex: 1 }}>
  ...
  return (
    <View style={s.dSection}>
      ...
      <TextInput
        style={[
          s.fInput,
          { height: 80, textAlignVertical: "top", marginBottom: 8 },
        ]}
      ...
      <TouchableOpacity
        style={[
          s.saveBtn,
          { marginBottom: 20 },
          (!newNote.trim() || saving) && { opacity: 0.5 },
        ]}
        onPress={onSave}
      ...
      >
        {saving ? (
          ...
          <Text style={s.saveBtnTxt}>+ Salvar Interação</Text>
        )}
      </TouchableOpacity>
...
function DossierFinance({
  ...
  return (
    <View style={s.dSection}>
      ...
      >
        <Text style={[s.saveBtnTxt, { color: "#f5c853" }]}>
        ...
        <View style={s.finForm}>
          ...
          <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
            {["PENDENTE", "PAGO", "CANCELADO"].map((st) => (
              <TouchableOpacity
                ...
                style={[
                  s.typeBtn,
                  financeForm.status === st && s.typeBtnActive,
                  { flex: 1 },
                ]}
              ...
              >
                <Text
                  style={[
                    s.typeBtnTxt,
                    financeForm.status === st && { color: "#f5c853" },
                    { fontSize: 11 },
                  ]}
                >
                  {st}
          ...
          >
            {saving ? (
              ...
              <Text style={s.saveBtnTxt}>Salvar Lançamento</Text>
            )}
          </TouchableOpacity>
        </View>
      )}
          ...
          <View key={r.id} style={s.finRecord}>
            ...
            <View style={{ alignItems: "flex-end", gap: 4 }}>
              <Text
                style={[
                  s.finRecordAmt,
                  { color: r.status === "PAGO" ? "#10b981" : "#f5c853" },
                ]}
              >
                {currency(r.amount)}
              </Text>
        ...
        {loading ? (
          ...
          <View style={{ gap: 12, alignItems: "center", paddingVertical: 10 }}>
            <Text
              style={[
                s.insightTxt,
                { color: "#ff453a", textAlign: "center", fontWeight: "bold" },
              ]}
            >
              Você atingiu o limite de 5 insights do Plano START.
            </Text>
            ...
            >
              Faça upgrade para o Plano PRO para obter insights estratégicos de
              inteligência jurídica ilimitados para todos os seus clientes.
            </Text>
            <TouchableOpacity
          ...
          <Text style={s.emptyTxt}>
            Toque em "Análise IA" para gerar um insight sobre este cliente.
          </Text>
        )}
      </View>
      {insight !== "LIMIT_REACHED" && (
        <Text style={s.insightDisclaimer}>
          ⚠️ A análise de IA é um auxílio estratégico e não substitui o
          julgamento profissional do advogado.
        </Text>
      )}
    </View>
  );
...
function DossierCases({ cases, loading }) {
  const statusColor = (st) =>
    st === "ABERTO" ? "#10b981" : st === "FECHADO" ? "#ff453a" : "#f5c853";
  return (
    <View style={s.dSection}>
        ...
        cases.map((c) => (
          <View key={c.id} style={s.caseItem}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.caseArea}>
                {c.area_atuacao || "Geral"} • {fmtDate(c.created_at)}
              </Text>
            </View>
            <View
              style={[
            ...
            >
              <Text style={[s.statusPillTxt, { color: statusColor(c.status) }]}>
                {c.status}
              </Text>
            </View>
          </View>
