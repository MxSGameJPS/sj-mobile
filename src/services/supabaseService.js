...
export const supabaseService = {
  ...
  async signIn(email, password) {
    try {
      ...
      if (!response.ok) {
        console.warn('[SupabaseService] signIn falhou. Status:', response.status, 'Payload:', JSON.stringify(data));
        const errMsg = data.error_description || data.error || data.msg || data.error_code || 'Erro ao realizar login';
      ...
      console.log('[SupabaseService] signIn concluído. Role identificada:', role);
      return {
        session: {
          accessToken,
  ...
  async checkOabExists(oab, estado) {
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/check_oab_exists`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'apikey': SUPABASE_ANON_KEY,
  ...
  async signUp(userData) {
    const { email, password, name, phone, role, oab, estado, origem_descoberta } = userData;
    try {
      ...
      if (!signupRes.ok) {
        throw new Error(signupData.error_description || signupData.error || signupData.message || 'Erro ao criar conta de autenticação');
      }
      ...
      const rpcRes = await fetch(`${SUPABASE_URL}/rest/v1/rpc/create_profile_public`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'apikey': SUPABASE_ANON_KEY,
      ...
      if (!rpcRes.ok) {
        ...
        throw new Error(rpcError.message || 'Erro ao salvar perfil no banco de dados.');
      }
  ...
  async getUserRole(userId, accessToken) {
    ...
    try {
      ...
      const adminRes = await fetch(`${SUPABASE_URL}/rest/v1/admins?id=eq.${userId}&select=role`, { headers });
      if (adminRes.ok) {
      ...
      const lawyerRes = await fetch(`${SUPABASE_URL}/rest/v1/advogados?id=eq.${userId}&select=id`, { headers });
      if (lawyerRes.ok) {
      ...
      const clientRes = await fetch(`${SUPABASE_URL}/rest/v1/clientes?id=eq.${userId}&select=id`, { headers });
      if (clientRes.ok) {
  ...
  async getLawyerInterests(userId, accessToken) {
    ...
    try {
      ...
      if (!iRes.ok) throw new Error(`Erro ao buscar interesses: ${iRes.status}`);
      ...
      if (!cRes.ok) throw new Error(`Erro ao buscar casos dos interesses: ${cRes.status}`);
  ...
  async getLawyerCases(userId, accessToken) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Erro ao buscar casos do advogado: ${res.status}`);
  ...
  async getMarketplaceCases(accessToken) {
    ...
    try {
      ...
      const { data, error } = await scopedSupabase
        .from('casos')
        .select('id, cliente_id, advogado_id, titulo, descricao, status, area_atuacao, valor_proposto, created_at, updated_at, anexos, chat_started, negotiating_lawyers, cidade, estado')
        .is('advogado_id', null)
  ...
  async getClientProfile(userId, accessToken, email = null) {
    ...
    try {
      ...
      let response = await fetch(`${SUPABASE_URL}/rest/v1/clientes?id=eq.${userId}&select=*`, { headers });
      if (response.ok) {
        const data = await response.json();
      ...
      if (email) {
        const responseEmail = await fetch(`${SUPABASE_URL}/rest/v1/clientes?email=eq.${email}&select=*`, { headers });
        if (responseEmail.ok) {
  ...
  async getClientCases(userId, accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/casos?cliente_id=eq.${userId}&select=*`, { headers });
  ...
  async getLawyer(lawyerId, accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/advogados?id=eq.${lawyerId}&select=*`, { headers });
  ...
  async createCase(caseData, accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/casos`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          ...caseData,
  ...
  async updateCase(caseId, updateData, accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/casos?id=eq.${caseId}`, {
        method: 'PATCH',
        headers,
  ...
  async updateClientProfile(userId, profileData, accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/clientes?id=eq.${userId}`, {
        method: 'PATCH',
        headers,
  ...
  async updateAuthPassword(password, accessToken) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error_description || data.error || 'Erro ao atualizar senha');
      }
      return data;
    } catch (error) {
  ...
  async getCaseMessages(caseId, accessToken, interestId = null) {
    ...
    try {
      const filter = interestId ? `caso_id=eq.${caseId}&interest_id=eq.${interestId}` : `caso_id=eq.${caseId}&interest_id=is.null`;
      const response = await fetch(`${SUPABASE_URL}/rest/v1/mensagens?${filter}&select=*&order=created_at.asc`, { headers });
  ...
  async sendCaseMessage(messageData, accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/mensagens`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          ...messageData,
  ...
  async uploadCaseAttachment(localUri, fileName, mimeType, accessToken) {
    try {
      ...
      if (!uploadResponse.ok) {
        throw new Error(uploadData.message || 'Erro no upload para o Supabase Storage');
      }
  ...
  async getLawyersList(accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/advogados?select=*`, { headers });
  ...
  async getOfficesList(accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/escritorios?select=*`, { headers });
  ...
  async getCaseInterests(clientId, accessToken) {
    ...
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/rest/v1/case_interests?${filter}&select=*&order=created_at.desc`, { headers });
  ...
  async respondToInterest(interestId, caseId, lawyerId, action, accessToken) {
    ...
    try {
      if (action === 'DECLINE') {
        // 1. Atualiza status do interesse para DECLINED
        const resInt = await fetch(`${SUPABASE_URL}/rest/v1/case_interests?id=eq.${interestId}`, {
          method: 'PATCH',
          headers,
          body: JSON.stringify({ status: 'DECLINED' }),
        });
        if (!resInt.ok) throw new Error('Erro ao recusar interesse');

        // 2. Criar notificação para o advogado
        await fetch(`${SUPABASE_URL}/rest/v1/notificacoes`, {
          ...
          body: JSON.stringify({
            ...
            mensagem: 'O cliente decidiu não prosseguir com a negociação neste caso.',
            lida: false,
            tipo: 'RECUSA',
      ...
      if (action === 'ACCEPT') {
        // 1. Atualiza status do interesse para NEGOTIATING
        const resInt = await fetch(`${SUPABASE_URL}/rest/v1/case_interests?id=eq.${interestId}`, {
          method: 'PATCH',
          headers,
          body: JSON.stringify({ status: 'NEGOTIATING' }),
        });
        if (!resInt.ok) throw new Error('Erro ao aceitar negociação');

        // 2. Atualizar status do caso para NEGOCIANDO
        const resCase = await fetch(`${SUPABASE_URL}/rest/v1/casos?id=eq.${caseId}`, {
          method: 'PATCH',
          headers,
          body: JSON.stringify({
            status: 'NEGOCIANDO',
        ...
        if (!resCase.ok) throw new Error('Erro ao atualizar status do caso');

        // 3. Criar notificação para o advogado
        await fetch(`${SUPABASE_URL}/rest/v1/notificacoes`, {
          method: 'POST',
          headers,
          body: JSON.stringify({
            ...
            mensagem: 'O cliente aceitou sua proposta. Vocês estão em fase de negociação.',
            lida: false,
            tipo: 'NEGOCIACAO',
            meta: JSON.stringify({ case_id: caseId }),
            created_at: new Date().toISOString(),
      ...
      if (action === 'HIRE') {
        // 1. Verificar saldo do advogado (precisa de 3 Juris)
        const lawyerRes = await fetch(`${SUPABASE_URL}/rest/v1/advogados?id=eq.${lawyerId}&select=balance`, {
          headers: {
            'apikey': SUPABASE_ANON_KEY,
            'Authorization': `Bearer ${accessToken}`,
            'Content-Type': 'application/json',
        ...
        if (!lawyerRes.ok) throw new Error('Erro ao obter dados do advogado');
        const lawyers = await lawyerRes.json();
        const lawyer = lawyers && lawyers.length > 0 ? lawyers[0] : null;
        if (!lawyer) throw new Error('Advogado não encontrado');

        const balance = lawyer.balance || 0;
        if (balance < 3) {
          throw new Error('O advogado não possui saldo de Juris suficiente para ser contratado (mínimo 3 Juris).');
        }

        // 2. Debitar 3 Juris do advogado
        const newBalance = balance - 3;
        const resBalance = await fetch(`${SUPABASE_URL}/rest/v1/advogados?id=eq.${lawyerId}`, {
          method: 'PATCH',
          headers,
          body: JSON.stringify({ balance: newBalance }),
        });
        if (!resBalance.ok) throw new Error('Erro ao atualizar saldo do advogado');

        // 3. Mudar status do interesse para HIRED
        const resInt = await fetch(`${SUPABASE_URL}/rest/v1/case_interests?id=eq.${interestId}`, {
          method: 'PATCH',
          headers,
          body: JSON.stringify({ status: 'HIRED' }),
        });
        if (!resInt.ok) throw new Error('Erro ao atualizar interesse para contratado');

        // 4. Recusar outros interesses para este caso
        await fetch(`${SUPABASE_URL}/rest/v1/case_interests?case_id=eq.${caseId}&id=neq.${interestId}`, {
          method: 'PATCH',
          headers,
          body: JSON.stringify({ status: 'DECLINED' }),
        });

        // 5. Vincular advogado ao caso e mudar status do caso para CONTRATADO
        const resCase = await fetch(`${SUPABASE_URL}/rest/v1/casos?id=eq.${caseId}`, {
          method: 'PATCH',
          headers,
          body: JSON.stringify({
        ...
        if (!resCase.ok) throw new Error('Erro ao vincular advogado ao caso');
        ...
        await fetch(`${SUPABASE_URL}/rest/v1/notificacoes`, {
          ...
          body: JSON.stringify({
            ...
            mensagem: 'Parabéns! O cliente contratou você para o caso.',
            lida: false,
            tipo: 'CONTRATACAO',
            meta: JSON.stringify({ case_id: caseId }),
            created_at: new Date().toISOString(),
  ...
  async updateCase(caseId, caseData, accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/casos?id=eq.${caseId}`, {
        method: 'PATCH',
        headers,
        body: JSON.stringify({ ...caseData, updated_at: new Date().toISOString() }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || 'Erro ao atualizar caso');
      return data && data.length > 0 ? data[0] : data;
    } catch (error) {
      console.error('[SupabaseService] Erro no updateCase:', error);
      throw error;
  ...
  async getNotifications(userId, accessToken) {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/notificacoes?user_id=eq.${userId}&order=created_at.desc`, { headers });
  ...
  async markNotificationRead(notificationId, accessToken) {
    ...
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/notificacoes?id=eq.${notificationId}`, {
        method: 'PATCH',
        headers,
