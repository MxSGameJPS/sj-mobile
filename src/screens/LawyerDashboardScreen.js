...
import { View, StyleSheet, StatusBar, Text, ActivityIndicator, ScrollView, RefreshControl, TouchableOpacity, TextInput, Modal, Platform, Alert, Clipboard } from 'react-native';
      ...
      try {
        ...
        if (profileJson.success && profileJson.data) {
          ...
          setLawyerProfile({
            ...
            oab_verification_status: profile.oab_verification_status || 'PENDING',
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (json.success) {
                Alert.alert('Sucesso', 'Interesse manifestado com sucesso! O cliente foi notificado.');
                fetchData();
              } else {
                if (res.status === 402 || json.message?.includes('Saldo insuficiente')) {
                  triggerNoticeModal(
                    'juris',
                    'Saldo insuficiente de Juris.\n\nPor razões de conformidade com as diretrizes das lojas, a aquisição de novos créditos de utilização (Juris) é realizada exclusivamente através do nosso portal de gestão no navegador web.'
                  );
                } else {
                  Alert.alert('Erro', json.message || 'Erro ao manifestar interesse');
                }
              }
            } catch (err) {
              console.warn(err);
      ...
      case 'Marketplace':
        ...
        return (
          <MarketplaceTab 
            ...
            onJurisPress={() => {
              console.log('[LawyerDashboardScreen] Card Saldo Juris pressionado');
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalHeader}>
              <View style={styles.modalTitleRow}>
                <Feather 
                  name={noticeModalType === 'juris' ? 'award' : noticeModalType === 'plan' ? 'star' : 'info'} 
                  size={18} 
                  color="#f5c853" 
                  style={{ marginRight: 8 }} 
                />
                <Text style={styles.modalTitle}>
                  {noticeModalType === 'juris' ? 'Saldo de Juris' : noticeModalType === 'plan' ? 'Gerenciar Plano' : 'Portal do Advogado'}
                </Text>
              </View>
            ...
            <View style={styles.modalBody}>
              <Text style={styles.modalText}>
                {noticeModalMessage ? noticeModalMessage : (
              ...
              <View style={styles.linkContainer}>
                <Text style={styles.linkTitle}>Acesse de qualquer navegador:</Text>
