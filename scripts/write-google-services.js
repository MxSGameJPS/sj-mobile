...
function writeFileFromBase64(envVarName, targetPath) {
  ...
  if (!b64) {
    console.log(
      `[prebuild] Environment variable ${envVarName} not set — skipping file write.`,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `[prebuild] Wrote file ${targetPath} from env ${envVarName} (size=${buffer.length} bytes)`,
    );
  } catch (err) {
...
try {
  ...
  console.log(
    "[prebuild] ENV keys available:",
    Object.keys(process.env)
      .filter((k) => k.includes("GOOGLE") || k.includes("EAS"))
      .join(", "),
  );
} catch (e) {
  // ignore
